...
CURRENT_DIRECTORY = "/media/ocean/大白菜U盘/GAN_study/ProteinGAN-master"

flags.DEFINE_string('dataset', DATASET + '/Length_512_lab_test_37_v2',
                    'Dataset to use for training. [mnist, /Length_512_lab_test_37_v2 cifar10]')
...
flags.DEFINE_string('name', 'test', 'A name of the model which will be used to construct folder name for weights1')
...
flags.DEFINE_string('weights_dir', os.path.join(CURRENT_DIRECTORY, 'weights'.replace("\\", os.sep)),
                    'Location where all weights1 should be saved')
...
if DATASET == 'image':
    ...
    flags.DEFINE_integer('embedding_height', 58, 'c')
    ...
    flags.DEFINE_string('variable_path', '../../../bert/weights1/tpu_fine_tuned/model.ckpt-1030000',
                        'Path of the checkpoint file where weights1 are stored for network part that '
